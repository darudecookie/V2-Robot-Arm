...
class Trajectory_Controller(Node):
    def __init__(self):
        ...
        self.system_diagnostic_sub = self.create_subscription(SystemDiagnosticInfo, "system_diagnostic_info", self.update_estop_jointhold_from_system, 10)
        self.Target_Cartesian_pub = self.create_publisher(TargetCartesian, "target_cartesian", 10)
        self.Para_Traj_action_server = rclpy.action.ActionServer(self, ParametricTrajectoryInfo, "parametric_tracjectory_info", self.parametric_trajector_callback)
    ...
    def update_output_from_system(self, msg):
        if msg.trajectory_controller == 1:
            self.should_output= 1
            self.get_logger().info("restarting trajectoy output")
        else:
            self.should_output = -1
            self.get_logger().info("pausing trajectoy output")
